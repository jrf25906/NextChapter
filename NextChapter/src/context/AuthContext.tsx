...
export function AuthProvider({ children, initialUser }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    const getInitialSession = async () => {
      try {
        // In demo mode, just set loading to false without making actual API calls
        if (
          !process.env.EXPO_PUBLIC_SUPABASE_URL ||
          process.env.EXPO_PUBLIC_SUPABASE_URL === "https://demo.supabase.co"
        ) {
          console.warn(
            "Running in demo mode - authentication features are limited",
          );
          setUser(null);
        ...
        // In case of error, assume no user is logged in
        setUser(null);
        setIsEmailVerified(false);
      } finally {
        setIsLoading(false);
    ...
    const {
      ...
      if (event === "SIGNED_IN") {
        ...
        if (
          ...
          setTimeout(async () => {
            const enrolled =
              await BiometricService.promptForBiometricEnrollment(
                session.user.id,
              );
            if (enrolled) {
  ...
  const enableBiometric = async (): Promise<boolean> => {
    ...
    try {
      const enrolled = await BiometricService.promptForBiometricEnrollment(
        user.id,
      );
      if (enrolled) {
        setIsBiometricEnabled(true);
